...
fn main() {
    ...
    let supported_by: Vec<HashSet<usize>> = rocks.iter().map(|rock| {
        match rock.t {
    ...
    let supporting: Vec<HashSet<usize>> = rocks.iter().map(|rock| {
        match rock.t {
            X => (0..rock.l).flat_map(|d| if let Occupied{id} = map[rock.p.2 + 1][rock.p.0 + d][rock.p.1] {Some(id)} else {None}).collect(),
            Y => (0..rock.l).flat_map(|d| if let Occupied{id} = map[rock.p.2 + 1][rock.p.0][rock.p.1 + d] {Some(id)} else {None} ).collect(),
            Z => if let Occupied{id} = map[rock.p.2 + rock.l][rock.p.0][rock.p.1] {HashSet::from([id])} else {HashSet::new()},
        }
    }).collect();

    let ans: i64 = rocks.iter().enumerate().map(|(i, _)| {
        let mut s_count: Vec<usize> = supported_by.iter().map(|s| s.len()).collect();
        let mut deq = VecDeque::new();
        let mut count = 0;
        deq.push_back(i);
        while let Some(i) = deq.pop_front() {
            for x in supporting[i].iter() {
                s_count[*x] -= 1;
                if s_count[*x] == 0 {
                    deq.push_back(*x);
                    count += 1;
