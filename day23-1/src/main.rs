...
fn sch(x: usize, y: usize, visited: &mut Vec<Vec<bool>>, map: &Vec<Vec<u8>>) -> i32 {
    ...
    let max_steps = match map[x][y] {
        ...
        b'.' => DIRECTIONS
            ...
            .max()
            .unwrap(),
        b'^' => try_move(x, y, &DIRECTIONS[0], map, visited),
        b'v' => try_move(x, y, &DIRECTIONS[1], map, visited),
        b'<' => try_move(x, y, &DIRECTIONS[2], map, visited),
        b'>' => try_move(x, y, &DIRECTIONS[3], map, visited),
        _ => panic!(),
    };
    visited[x][y] = false;
    if max_steps > 0 {
